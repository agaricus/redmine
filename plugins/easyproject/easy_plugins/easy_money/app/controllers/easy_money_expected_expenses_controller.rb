class EasyMoneyExpectedExpensesController < ApplicationController
  ...
  def index
    ...
    @money_entity_type = 'expected_expenses'
  ...
  def new
    @easy_money_object = EasyMoneyExpectedExpense.new
  ...
  def create
    ...
    if @easy_money_object.save
      ...
      Redmine::Hook.call_hook(:controller_easy_money_expected_expenses_create_after_save, {:expected_expense => @easy_money_object, :attachments => attachments, :params => params})
  ...
  def update
    ...
    if @easy_money_object.save
      attachments = Attachment.attach_files(@easy_money_object, params[:attachments])
      Redmine::Hook.call_hook(:controller_easy_money_expected_expenses_update_after_save, {:expected_expense => @easy_money_object, :attachments => attachments, :params => params})
  ...
  def inline_update
    expected_expense = @entity.expected_expenses.first || @entity.expected_expenses.new(:name => l(:label_easy_money_new_expense_text), :description => l(:label_easy_money_new_expense_description), :spent_on => Date.current)

    if @project.easy_money_settings.expected_count_price == 'price1'
      price1 = params[:expected_expenses][:price].to_f
      price2 = EasyMoneyEntity.compute_price2(@project, price1)
    else
      price2 = params[:expected_expenses][:price].to_f
      price1 = EasyMoneyEntity.compute_price1(@project, price2)
    end

    expected_expense.price1 = price1
    expected_expense.vat = @project.easy_money_settings.vat.to_f
    expected_expense.price2 = price2
    expected_expense.save(:validate => false)
  ...
  def find_easy_money_object
    @easy_money_object = EasyMoneyExpectedExpense.find(params[:id])
