class EasyMoneyOtherRevenuesController < ApplicationController
  ...
  def index
    ...
    @money_entity_type = 'other_revenues'
  ...
  # GET /easy_money_other_revenues/:id
  # GET /easy_money_other_revenues/:id/show
  def show
    respond_to do |format|
      format.html
      format.api
  ...
  # GET /easy_money_other_revenues/new
  def new
    @easy_money_object = EasyMoneyOtherRevenue.new
  ...
  # POST /easy_money_other_revenues
  def create
    @easy_money_object = EasyMoneyOtherRevenue.new
    @easy_money_object.safe_attributes = params[:easy_money] if params[:easy_money]
    ...
    if @easy_money_object.save
      ...
      Redmine::Hook.call_hook(:controller_easy_money_other_revenues_create_after_save, {:other_revenue => @easy_money_object, :attachments => attachments, :params => params})
  ...
  # GET /easy_money_other_revenues/:id/edit
  def edit
  ...
  # PUT /easy_money_other_revenues/:id
  def update
    @easy_money_object.safe_attributes = params[:easy_money] if params[:easy_money]

    if @easy_money_object.save
      attachments = Attachment.attach_files(@easy_money_object, params[:attachments])
      Redmine::Hook.call_hook(:controller_easy_money_other_revenues_update_after_save, {:other_revenue => @easy_money_object, :attachments => attachments, :params => params})
  ...
  # DELETE /easy_money_other_revenues/:id
  def destroy
  ...
  def inline_update
    other_revenue = @entity.other_revenues.first || @entity.other_revenues.new(:name => l(:label_easy_money_new_revenue_text), :description => l(:label_easy_money_new_revenue_description), :spent_on => Date.current)

    if @project.easy_money_settings.expected_count_price == 'price1'
      price1 = params[:other_revenues][:price].to_f
      price2 = EasyMoneyEntity.compute_price2(@project, price1)
    else
      price2 = params[:other_revenues][:price].to_f
      price1 = EasyMoneyEntity.compute_price1(@project, price2)
    end

    other_revenue.price1 = price1
    other_revenue.vat = @project.easy_money_settings.vat.to_f
    other_revenue.price2 = price2
    other_revenue.save(:validate => false)
  ...
  def find_easy_money_object
    @easy_money_object = EasyMoneyOtherRevenue.find(params[:id])
